#!/usr/bin/env python3
"""
LHM Settings Refactor - Groups 200+ flat fields into nested configs.
"""

import re
from pathlib import Path

MAIN_MODEL = Path(r"C:\Users\enock\Downloads\deepseek_python_20260707_a6bd19.py")
BACKUP = MAIN_MODEL.with_suffix(".py.settings_backup")

if not BACKUP.exists():
    BACKUP.write_text(MAIN_MODEL.read_text(encoding="utf-8"), encoding="utf-8")

content = MAIN_MODEL.read_text(encoding="utf-8")

# Remove old Settings class
old_settings_pattern = r'class Settings\(BaseSettings\):.*?(?=\n(?:class |# ={3,}|[A-Z][a-zA-Z]*\(|def |async def |@|CONFIG = ))'
match = re.search(old_settings_pattern, content, re.DOTALL)
if match:
    old_settings = match.group(0)
    content = content.replace(old_settings, "")
    print("Removed old flat Settings class")

# Insert new nested Settings before CONFIG = Settings()
new_settings = '''
...
# Insert before CONFIG = Settings()
content = content.replace("CONFIG = Settings()", new_settings + "CONFIG = Settings()")

MAIN_MODEL.write_text(content, encoding="utf-8")
print("Settings refactored into nested configs")
